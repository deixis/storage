...
type Aggregate interface {
	...
	// MarshalSnapshot encodes the receiver into a full snapshot form and
	// returns the result.
	MarshalSnapshot() (data []byte, err error)
	// UnmarshalSnapshot must be able to decode the form generated by MarshalSnapshot.
	// UnmarshalSnapshot must copy the data if it wishes to retain the data
	// after returning.
	UnmarshalSnapshot(data []byte, version uint64) error
}
...
func (tx *aggregateReadTransaction) load(a Aggregate, meta *StreamMetadata, version uint64) error {
	...
	case nil:
		...
		if err := a.UnmarshalSnapshot(recordedSnap.Data, from); err != nil {
			return err
		}
	case ErrNoSnapshot:
