...
func TestPrefix(t *testing.T) {
	...
	for i, test := range table {
		out := badger.Prefix(test.A, test.B)
		if bytes.Compare(test.Expect, out) != 0 {
			t.Errorf("#%d - Expect to get %s, but got %s", i, test.Expect, out)
		}
	}
}

func openDB(t *testing.T) *badger.Store {
	store, err := badger.Open(path.Join("db", t.Name()))
	if err != nil {
		log.Fatal(err)
	}
	return store
